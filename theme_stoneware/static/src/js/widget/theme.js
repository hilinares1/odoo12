...
Theme.include({
    xmlDependencies: (Theme.prototype.xmlDependencies || [])
        .concat(['/theme_stoneware/static/src/xml/website_editor.xml']),
    events: {
    ...
    _onColorClick: function (ev) {
        ...
        colorpicker.on('colorpicker:saved', this, function (ev) {
            ...
            // TODO improve to be more efficient
                ...
                if(colorName === 'stoneware'){
		            var updatedFileContent = "$as-theme:" + colorValue + ";";
		            var baseURL = '/stoneware_customize/static/src/scss/options/colors/color_picker.scss';                                
                     		self._rpc({
							    ...
							    self._rpc({
								    ...
								    params: {
								        url: '/stoneware_customize/static/src/scss/options/colors/color_picker.scss',
