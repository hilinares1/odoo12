...
class AccountReport(models.AbstractModel):
    ...
    def _get_templates(self):
        return {
                'main_template': 'gts_account_report.main_template',
                'main_table_header_template': 'gts_account_report.main_table_header',
                'line_template': 'gts_account_report.line_template',
                'footnotes_template': 'gts_account_report.footnotes_template',
                'search_template': 'gts_account_report.search_template',
        }
    ...
    def open_general_ledger(self, options, params=None):
        ...
        ctx = self.env.context.copy()
        ctx.pop('id', '')
        action = self.env.ref('gts_account_report.action_account_report_general_ledger').read()[0]
    ...
    def get_html(self, options, line_id=None, additional_context=None):
        ...
        * if line_id is set, the template used will be the line_template
        ...
        render_template = templates.get('main_template', 'gts_account_report.main_template')
        if line_id is not None:
            render_template = templates.get('line_template', 'gts_account_report.line_template')
    ...
    def get_html_footnotes(self, footnotes):
        template = self._get_templates().get('footnotes_template', 'gts_account_report.footnotes_template')
    ...
    def get_pdf(self, options, minimal_layout=True):
        ...
        body = self.env['ir.ui.view'].render_template(
            "gts_account_report.print_template",
            values=dict(rcontext),
        )
