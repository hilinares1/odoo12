    ...
    def send_whatsapp_automatic(self):
        for inv in self:
            ...
            if self._get_whatsapp_server() and self._get_whatsapp_server().status == 'authenticated':
                ...
                vals = WhatsappComposeMessage._prepare_mail_message(self.env.user.id, AllchatIDs, inv and inv.id,  'account.invoice', body, message_data, subject, partners.ids, attachment_ids, send_message, status)
