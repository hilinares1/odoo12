...
odoo.define('pos_retail.promotion', function (require) {
    ...
    models.Order = models.Order.extend({
            ...
            if (rules) {
                for (var i = 0; i < rules.length; i++) {
                    ...
                    if (promotion.method == 'only_one') {
                        if (rule.product_id && total_qty_by_product[rule.product_id[0]] && rule.qty_apply <= total_qty_by_product[rule.product_id[0]]) {
                            can_apply = true;
                            break;
                        }
                    } else {
        ...
        compute_multi_buy: function (promotion) { // TODO: multi_buy
            var rule_applied = {};
            var total_qty_by_product = this.product_quantity_by_product_id();
            ...
            if (rule_applied && product_discount) {
                for (var product_id in rule_applied) {
                    ...
                    if (total_qty >= qty_apply) {
                        ...
                        promotion_reason = product.display_name + ' have Quantity bigger than or equal ' + rule_applied[product_id].qty_apply + ', set price is ' + this.pos.gui.chrome.format_currency(rule_applied[product_id].list_price);
                        if (price_discount < 0) {
