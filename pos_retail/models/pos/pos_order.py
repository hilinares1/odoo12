    ...
    def create_from_ui(self, orders):
        ...
        for o in orders:
            ...
            if config.push_order_no_wait:
                ...
                self.env.cr.execute("select id from pos_order where pos_reference='%s'" % data['name'])
                orders_exist = self.env.cr.fetchall()
                if len(orders_exist) == 0:
                    if to_invoice:
                        self._match_payment_to_invoice(data)
                    queue_order_ids.append(self._process_order(data).id)
        if push_order_no_wait and queue_order_ids:
