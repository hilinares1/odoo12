...
_HIERARCHY_TUP = [('hierarchy', "Hierarchy")]
...
class Base(models.AbstractModel):
    ...
    @api.model
    def read_hierarchy(self, row_fields, col_field, cell_field, domain=None, range=None):
        """
        ...
        column_info = self._hierarchy_column_info(col_field, range)
        ...
        rows = self._hierarchy_get_row_headers(row_fields, groups, key=row_key)
        ...
        for group in groups:
            ...
            cell_map[row][col] = self._hierarchy_format_cell(group, cell_field)

        # pre-build whole hierarchy, row-major, h = len(rows), w = len(cols),
        # each cell is
        #
        ...
        for r in rows:
            ...
            for c in cols:
                ...
                if it: # accumulated cell exists, just use it
                    ...
                    row.append(self._hierarchy_make_empty_cell(d))
    ...
    def _hierarchy_make_empty_cell(self, cell_domain):
        return {'size': 0, 'domain': cell_domain, 'value': 0}

    def _hierarchy_format_cell(self, group, cell_field):
        return {
            'size': group['__count'],
    ...
    def _hierarchy_get_row_headers(self, row_fields, groups, key):
        seen = {}
        rows = []
        for cell in groups:
            k = key(cell)
    ...
    def _hierarchy_column_info(self, name, range):
        """
        :param str name:
        :param range:
        ...
        context_anchor = self.env.context.get('hierarchy_anchor')
        ...
        if field.type == 'selection':
            ...
            r = self._hierarchy_range_of(span, step, anchor)
            pagination = self._hierarchy_pagination(field, span, step, anchor)
            return ColumnMetadata(
                ...
                values=[{
                        ...
                        'domain': ['&',
                                   ...
                                   (name, '<', field.to_string(d + self._hierarchy_step_by(step)))],
                        'is_current': self._hierarchy_date_is_current(field, span, step, d)
                    } for d in r.iter(step)
                ],
                format=lambda a: a and a[0],
            )
        else:
            raise ValueError(_("Can not use fields of type %s as hierarchy columns") % field.type)

    @api.model
    def read_hierarchy_domain(self, field, range):
        """ JS hierarchy view may need to know the "span domain" of the hierarchy before
        ...
        raise UserError(_("Can not use fields of type %s as hierarchy columns") % field.type)
    ...
    def _get_date_column_label(self, date, field, span, step):
        ...
        if step == 'week':
            label = _("Week %(weeknumber)s\n%(week_start)s - %(week_end)s") % {
                ...
                'week_end': format_date(self.env, date + self._hierarchy_step_by(step) - relativedelta(days=1), locale, "MMM\u00A0dd")
            }
        return ("%s/%s" % (field.to_string(date), field.to_string(date + self._hierarchy_step_by(step))), label)
    ...
    def _hierarchy_pagination(self, field, span, step, anchor):
        if field.type == 'date':
            today = field.from_string(field.context_today(self))
            diff = self._hierarchy_step_by(span)
            period_prev = field.to_string(anchor - diff)
            period_next = field.to_string(anchor + diff)
            return {
                'prev': {'hierarchy_anchor': period_prev, 'default_%s' % field.name: period_prev},
                'next': {'hierarchy_anchor': period_next, 'default_%s' % field.name: period_next},
                'initial': {'hierarchy_anchor': field.to_string(today), 'default_%s' % field.name: field.to_string(today)}
            }
        return dict.fromkeys(['prev', 'initial', 'next'], False)

    def _hierarchy_step_by(self, span):
        return STEP_BY.get(span)

    def _hierarchy_range_of(self, span, step, anchor):
        return date_range(self._hierarchy_start_of(span, step, anchor),
                          self._hierarchy_end_of(span, step, anchor))

    def _hierarchy_start_of(self, span, step, anchor):
        if step == 'week':
            return anchor + START_OF_WEEK[span]
        return anchor + START_OF[span]

    def _hierarchy_end_of(self, span, step, anchor):
        if step == 'week':
            return anchor + END_OF_WEEK[span]
        return anchor + END_OF[span]

    def _hierarchy_start_of_period(self, span, step, anchor):
        if step == 'day':
            return anchor
        return anchor + START_OF[step]

    def _hierarchy_end_of_period(self, span, step, anchor):
        if step == 'day':
            return anchor
        return anchor + END_OF[step]

    def _hierarchy_date_is_current(self, field, span, step, date):
        today = field.from_string(field.context_today(self))
        if step == 'day':
            return today == date
        elif step in ['week', 'month']:
            return self._hierarchy_start_of_period(span, step, date) <= today < self._hierarchy_end_of_period(span, step, date)
        return False
