...
class AccountPayment(models.Model):
    ...
    def create(self, vals):
        if vals.get('payment_date') and not vals.get('period_id'):
            period_id = self.env['account.period'].search([('state', '!=', 'done'), ('date_start', '<=' , vals.get('payment_date')), ('date_stop', '>=', vals.get('payment_date')), ('company_id', '=', self.env.user.company_id.id)], limit=1) 
            if period_id:
