...
class stock_inv_report(models.AbstractModel):
    ...
    def get_sale_qty(self, product, warehouse_id,data):
        ...
        if data.location_id:
            params = (start_date, end_date, warehouse_id.id, move_type, data.location_id.id,data.production_id.id, product.id, state,
                      data.company_id.id)
        else:
    ...
    def get_lines(self,data):
        ...
        for warehouse_id in data.warehouse_ids:
            ...
            for product in product_ids:
                ...
                mrp_qty = self.get_mrp_qty(product, warehouse_id,data)
                adjust_qty_positive = self.get_pos_adjustment_qty(product, warehouse_id,data)
                adjust_qty_negative = self.get_neg_adjustment_qty(product, warehouse_id,data)
                ...
                if not data.is_zero:
                    if beginning_qty != 0 or received_qty != 0 or mrp_qty != 0 or sale_qty != 0 or  adjust_qty != 0 or ending_qty != 0:
                        lst.append({
    ...
    def get_before_incoming_qty(self,product, warehouse_id,data):
        ...
        start_date = str(data.start_date) + ' 00:00:00'
        if data.location_id:
            params = (start_date,warehouse_id.id, move_type, data.production_id.id,data.location_id.id, product.id, state,
                      data.company_id.id)
        else:
    ...
    def get_before_outgoing_qty(self, product, warehouse_id,data):
        ...
        if data.location_id:
            params = (start_date, warehouse_id.id, move_type, data.location_id.id,data.production_id.id, product.id, state,
                      data.company_id.id)
        else:
            params = (start_date, warehouse_id.id, move_type, product.id, state, data.company_id.id)
    ...
    def get_receive_qty(self, product, warehouse_id,data):
        ...
        if data.location_id:
            params = (start_date, end_date, warehouse_id.id, move_type, data.production_id.id,data.location_id.id, product.id, state,
                      data.company_id.id)
        else:
            params = (start_date, end_date, warehouse_id.id, move_type, product.id, state,data.company_id.id)
    ...
    @api.multi
    def get_mrp_qty(self, product, warehouse_id,data):
        state = 'done'
        ...
        if data.location_id:
            params = (start_date, end_date, warehouse_id.id, move_type,data.location_id.id, data.location_id.id, product.id, state,
                      data.company_id.id)
        else:
            params = (start_date, end_date, warehouse_id.id, move_type, product.id, state, data.company_id.id)
