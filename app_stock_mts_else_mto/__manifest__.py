# -*- coding: utf-8 -*-

# Created on 2019-05-26
# author: 广州尚鹏，https://www.sunpop.cn
# email: 300883@qq.com
# resource of Sunpop
# License LGPL-3.0 or later (http://www.gnu.org/licenses/lgpl).

# Odoo12在线用户手册（长期更新）
# https://www.sunpop.cn/documentation/user/12.0/zh_CN/index.html

# Odoo12在线开发者手册（长期更新）
# https://www.sunpop.cn/documentation/12.0/index.html

# Odoo10在线中文用户手册（长期更新）
# https://www.sunpop.cn/documentation/user/10.0/zh_CN/index.html

# Odoo10离线中文用户手册下载
# https://www.sunpop.cn/odoo10_user_manual_document_offline/
# Odoo10离线开发手册下载-含python教程，jquery参考，Jinja2模板，PostgresSQL参考（odoo开发必备）
# https://www.sunpop.cn/odoo10_developer_document_offline/

##############################################################################
#    Copyright (C) 2009-TODAY Sunpop.cn Ltd. https://www.sunpop.cn
#    Author: Ivan Deng，300883@qq.com
#    You can modify it under the terms of the GNU LESSER
#    GENERAL PUBLIC LICENSE (LGPL v3), Version 3.
#    See <http://www.gnu.org/licenses/>.
#
#    It is forbidden to publish, distribute, sublicense, or sell copies
#    of the Software or modified copies of the Software.
##############################################################################

{
    'name': "MTS else MTO, Stock enough MTS else MTO procurement",
    'version': '12.19.06.07',
    'author': 'Sunpop.cn',
    'category': 'Base',
    'website': 'https://www.sunpop.cn',
    'license': 'LGPL-3',
    'sequence': 2,
    'price': 98.00,
    'currency': 'EUR',
    'images': ['static/description/banner.png'],
    'depends': [
        'stock',
    ],
    'summary': """
    MTS MTO Advanct stock rule. make to stock plus make to order rule.
    Ready for purchase and manufacture. mts plus mto
    Product Take From Stock, if unavailable, Trigger Another Rule like MTO make to order.
    """,
    'description': """    
    Support Odoo 12, Enterprise and Community Edition
    Support 3 way for product pull rule.
    1. MTS,Take From Stock: the products will be taken from the available stock of the source location.
    2. MTO,Trigger Another Rule: the system will try to find a stock rule to bring the products in the source location. The available stock will be ignored.
    3. MTS+MTO,Take From Stock, if Forecasted Qty Unavailable, Trigger Another Rule: the products will be taken from the available stock of the source location.If there is no stock available, the system will try to find a  rule to bring the products in the source location.
    
    4. MTS + MTO and other rule can use in purchase or manufacture order.
    5. Multi-language Support.
    6. Multi-Company Support.
    ==========
    增强的 MTS + MTO 补货规则
    支持三种拉式补货
    1. MTS 从库存中获取：产品将从来源库位的可用库存中获取。
    2. MTO 触发其他规则: 系统将尝试从来源库位中匹配高级补货规则。可用库存将被忽略。
    3. MTS + MTO 从库存获取，如果在途数量不足则触发规则: 产品将优先尝试从来源库位的可用库存中获取。如果在途数量不足，系统将尝试从来源库位中匹配高级补货规则

    4. MTS + MTO 及所有拉规则都可用于采购及制造
    5. 多语言支持
    6. 多公司支持
    """,
    'data': [
        # 'security/*.xml',
        # 'security/*.csv',
        'data/stock_data.xml',
        # 'views/*.xml',
    ],
    'qweb': [
        'static/src/xml/*.xml',
    ],
    'demo': [],
    'test': [],
    'css': [],
    'js': [],
    'pre_init_hook': 'pre_init_hook',
    'post_init_hook': 'post_init_hook',
    'uninstall_hook': 'uninstall_hook',
    'installable': True,
    'application': True,
    'auto_install': False,
}
