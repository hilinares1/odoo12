...
class Stage(models.Model):
    ...
    def default_get(self, fields):
        ...
        if ctx.get('default_team_id') and not ctx.get('srm_team_mono'):
    ...
    requirements = fields.Text('Requirements', help="Enter here the internal requirements for this stage (ex: Vendor Sent Offer to Us). It will appear as a tooltip over the stage's name.")
    team_id = fields.Many2one('srm.team', string='Purchase Team', ondelete='set null',
    ...
    def _compute_team_count(self):
        for stage in self:
            stage.team_count = self.env['srm.team'].search_count([])
