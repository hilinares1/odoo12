...
class generic_tax_report(models.AbstractModel):
    ...
    def _sql_cash_based_taxes_ifrs(self):
        sql = """SELECT tax.id, SUM(CASE WHEN tax.type_tax_use = 'sale' THEN -"account_move_line".fal_tax_base_amount_group_curr ELSE "account_move_line".fal_tax_base_amount_group_curr END), SUM("account_move_line".fal_balance_group_curr) FROM account_tax tax, %s WHERE tax.id = "account_move_line".tax_line_id AND %s AND tax.tax_exigibility = 'on_payment' and "account_move_line".tax_exigible GROUP BY tax.id"""
        return sql

    def _sql_tax_amt_regular_taxes_ifrs(self):
        sql = """SELECT "account_move_line".tax_line_id, COALESCE(SUM("account_move_line".fal_debit_group_curr-"account_move_line".fal_credit_group_curr), 0)
    ...
    def _sql_net_amt_regular_taxes_ifrs(self):
        sql = """SELECT r.account_tax_id, COALESCE(SUM("account_move_line".fal_debit_group_curr-"account_move_line".fal_credit_group_curr), 0)
                 FROM %s
    ...
    def _compute_from_amls_ifrs(self, options, taxes, period_number):
        sql = self._sql_cash_based_taxes_ifrs()
        ...
        sql = self._sql_net_amt_regular_taxes_ifrs()
        query = sql % (tables, where_clause)
        self.env.cr.execute(query, where_params)
        results = self.env.cr.fetchall()
        for result in results:
            if result[0] in taxes:
                taxes[result[0]]['periods'][period_number]['net'] = result[1]
                taxes[result[0]]['show'] = True
        sql = self._sql_tax_amt_regular_taxes_ifrs()
        query = sql % (tables, where_clause)
        self.env.cr.execute(query, where_params)
        results = self.env.cr.fetchall()
        for result in results:
    ...
    def _get_lines(self, options, line_id=None):
        ...
        period_number = 0
        # Change Start
        # Need to call different method to get IFRS number
        if options.get('ifrs', False):
            self._compute_from_amls_ifrs(options, taxes, period_number)
        else:
        ...
        for period in options['comparison'].get('periods'):
            ...
            # Need to call different method to get IFRS number
            if options.get('ifrs', False):
                self.with_context(date_from=period.get('date_from'), date_to=period.get('date_to'))._compute_from_amls_ifrs(options, taxes, period_number)
            else:
