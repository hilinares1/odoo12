...
class report_account_general_ledger(models.AbstractModel):
    ...
    def _do_query_unaffected_earnings(self, options, line_id, company=None):
        ...
        # Change Start Here
        # We use the debit / credit / balance group curr instead
        elif options.get('ifrs'):
            select = select.replace('debit', 'fal_debit_group_curr').replace('credit', 'fal_credit_group_curr').replace('balance', 'fal_balance_group_curr')
        # End Here
    ...
    def _do_query_group_by_account(self, options, line_id):
        ...
        def build_converter(currency):
            def convert(amount):
                # Change Start Here
                # Do not convert if we use IFRS method
                if options.get("ifrs"):
                    return amount
                # Change End Here
                else:
    ...
    def _get_taxes(self, journal):
        ...
        if self.env.context.get('ifrs'):
            query = """
                SELECT rel.account_tax_id, SUM("account_move_line".fal_balance_group_curr) AS base_amount
        ...
        for tax in self.env['account.tax'].browse(ids):
            if self.env.context.get('ifrs'):
                self.env.cr.execute('SELECT sum(fal_debit_group_curr - fal_credit_group_curr) FROM ' + tables + ' '
