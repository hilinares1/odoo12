        ...
        if options.get("ifrs", False):
            select = """
                SELECT to_char("account_move_line".date, 'MM') as month,
                       ...
                       COALESCE(SUM("account_move_line".fal_balance_group_curr), 0) as balance,
                       COALESCE(SUM("account_move_line".fal_debit_group_curr), 0) as debit,
                       COALESCE(SUM("account_move_line".fal_credit_group_curr), 0) as credit,
                       j.id as journal_id,
