...
class FalAccountInvoice(models.TransientModel):
    ...
    def export_efaktur(self):
        # Data Tax
        cek_invoices = self.env['account.invoice'].browse(
                ...
                for order in invoices:
                    myData += [
                        [
                            ...
                            int(order.date_invoice and order.date_invoice.strftime('%m')),
                            order.date_invoice and order.date_invoice.strftime('%Y'),
                            order.date_invoice and order.date_invoice.strftime('%d/%m/%Y'),
                            order.npwp,
                            ...
                            '0',
                            '0',
                            order.number or order.origin or '',
                        ]
                    ]
                ...
                for order in invoices:
                    ...
                    myData += [
                        [
                            ...
                            order.faktur_pajak_id.category,
                            str(fp),
                            int(order.date_invoice.strftime('%m')),
                            order.date_invoice.strftime('%Y'),
                            order.date_invoice.strftime('%d/%m/%Y'),
                            str(npwp),
                            order.partner_id.name,
                            order.partner_id.street,
                            int(order.amount_untaxed),
